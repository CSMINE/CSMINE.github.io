<?php

namespace App;

use Carbon\Carbon;
use Illuminate\Database\Eloquent\Model;

class Game extends Model
{
    const STATUS_NOT_STARTED = 0;
    const STATUS_PLAYING = 1;
    const STATUS_PRE_FINISH = 2;
    const STATUS_FINISHED = 3;
    const STATUS_ERROR = 4;

    const STATUS_PRIZE_WAIT_TO_SENT = 0;
    const STATUS_PRIZE_SEND = 1;
    const STATUS_PRIZE_SEND_ERROR = 2;

    protected $fillable = ['rand_number'];

    public function winner()
    {
        return $this->belongsTo('App\User');
    }

    public function users()
    {
        return \DB::table('games')
            ->join('bets', 'games.id', '=', 'bets.game_id')
            ->join('users', 'bets.user_id', '=', 'users.id')
            ->where('games.id', $this->id)
            ->groupBy('users.username')
            ->select('users.*')
            ->get();
    }

    public static function gamesToday()
    {
        return self::where('status', self::STATUS_FINISHED)->where('created_at', '>=', Carbon::today())->count() + 55;
    }

    public static function gamesAll()
    {
        return self::where('status', self::STATUS_FINISHED)->count();
    }

    public static function usersToday()
    {
        return count(\DB::table('games')
            ->join('bets', 'games.id', '=', 'bets.game_id')
            ->join('users', 'bets.user_id', '=', 'users.id')
            ->where('games.created_at', '>=', Carbon::today())
            ->groupBy('users.username')
            ->select('users.username')->get()) + 75;
    }

    public static function usersAll()
    {
        return count(\DB::table('games')
            ->join('bets', 'games.id', '=', 'bets.game_id')
            ->join('users', 'bets.user_id', '=', 'users.id')
            ->groupBy('users.username')
            ->select('users.username')->get());
    }

    public static function maxPriceToday()
    {
        return ($price = self::where('created_at', '>=', Carbon::today())->max('price')) ? $price : 0;
    }

    public static function maxPriceAll()
    {
        return self::max('price');
    }

    public static function itemsToday()
    {
        return \DB::table('games')
            ->join('bets', 'games.id', '=', 'bets.game_id')
            ->join('users', 'bets.user_id', '=', 'users.id')
            ->where('games.created_at', '>=', Carbon::today())
            ->count('itemsCount');
    }

    public static function itemsAllDays()
    {
        return \DB::table('games')
            ->join('bets', 'games.id', '=', 'bets.game_id')
            ->join('users', 'bets.user_id', '=', 'users.id')
            ->count('itemsCount');
    }

	public static function maxPrice()
    {
        return round(self::max('price'));
    }

	public static function paidAll()
    {
		return number_format(round(self::sum('price')));
    }

    public function bets()
    {
        return $this->hasMany('App\Bet');
    }
}
